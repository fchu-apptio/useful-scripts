...
def search_environments( params ):
  print('Running search environments...')
  api = RouterApi(params.router, False)
  ...
  environments = EnvironmentsHelper.get_all_info_map(application, api.get_environment_by_application(application.get('id')), api)
  if params.name:
    environments = filter(lambda e: e.get('fqen') and params.name in e.get('fqen'), environments)
  if params.version:
    environments = filter(lambda e: e.get('versionBuildNumber') and params.version in e.get('versionBuildNumber'), environments)
...
def search_versions( params ):
  ...
  if params.custom=='bad':
    versions = VersionsHelper.filterBadVersions( versions, api )
