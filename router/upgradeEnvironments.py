...
parser.add_argument('--fqen', '-f', dest='fqen', type=str, required=False, help='Fully qualified environment name to upgrade')
parser.add_argument('--new-version', '-v', dest='version', type=str, required=True, help='Tag of the bff version to provision to')
parser.add_argument('--old-version', '-o', dest='old', type=str, required=False, help='Tag of the bff version to provision to')
...
if args.fqen:
  ...
  envVer = api.get_envVersions_by_env_and_app(environment.get('id'), application.get('id'))
  if not envVer or 'id' not in envVer:
    sys.exit("Could not find envVer for env {} and application {}".format(environment.get('id'), application.get('id')))
  
  output = api.put_envVersion_for_environments(args.version, [envVer.get('id')])
elif args.old:
  ...
  environments = map(lambda e: e.get('id'), filter(lambda ev: ev.get('versionBuildNumber') and args.old==ev.get('versionBuildNumber'), api.get_envVersions_by_app(application.get('id'))))
  output = api.put_envVersion_for_environments(args.version, list(environments))
